...
half4 CombinedShapeLightShared(half4 color, half4 mask, half2 lightingUV, half2 worldUV)
{
    ...
    finalOutput = color;
#else
    // Calculate overlapMask based on the overall intensity of shapeLight1
    float shapeLight1Intensity = dot(shapeLight1Modulate + shapeLight1Additive, half4(0.25, 0.25, 0.25, 0.25)); // Averaging the channels
    float overlapMask = step(0.4, shapeLight1Intensity); // Use a small threshold to avoid noise

    // Modify shapeLight2 to only be rendered where overlapMask is positive
    half4 modifiedShapeLight2Modulate = shapeLight2Modulate * overlapMask;
    half4 modifiedShapeLight2Additive = shapeLight2Additive * overlapMask;

    // Now combine the final modulate and additive values based on this condition
    half4 finalModulate = shapeLight0Modulate + shapeLight3Modulate + modifiedShapeLight2Modulate;
    half4 finalAdditive = shapeLight0Additive + shapeLight3Additive + modifiedShapeLight2Additive;

    // Calculate the final output as usual
